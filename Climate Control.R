...
#EDA
#correlation Coefficient of CO2 and temp
cor(as.numeric(climate$temp), climate$co2)

#scatterplot of Temp~CO2
qplot(temp, co2, data=climate, geom="point", xlab="Temperature", ylab="CO2 Concentration")
#The scatterplot does have the characteristics suggested by the correlation coefficient

#correlation coefficient of methane and temp
cor(as.numeric(climate$temp), climate$methane)

#scatterplot of Methane~Temp
qplot(temp, methane, data=climate, geom="point", xlab="Temperature", ylab="Methane Concentration")
#The scatterplot does have the characteristics suggested by the correlation coefficient

#Analysis:

#The response variable is Earth Temp (Monthly)
#Our explanatory variables are CO2 (ppm) and Methane (ppb)
#This is a observational study. Because we don't have more than one Earth to test on, it is only an
#observational study. Therefore, we can only conclude correlation, not causation. 

#Model:
#  Temp = beta0 + beta1*CO2 + beta2*Methane + epsilon, where epsilon ~ N(0, sigma2)
#Assumptions: That the data has a linear correlation, with constant variance. 
#             That the variables have an independent effect.


#Estimate: Beta0.hat, Beta1.hat, Beta2.hat, sigma2 and standard errors
...
#Beta1.hat is the estimated partial slope of co2 (The slope of co2 if we hold methane constant)
#For a one unit change in CO2 (ppm), there is an estimated expected increase of 0.741 
#(.01 degrees C) in monthly earth temperature, given that methane concentration is constant. 

#Beta2.hat is the estimated partial slope of methane ( The slope of methane if we hold co2 
#constant). For a one unit change in Methane (ppb), there is an estimated expected increase 
#of 0.09444 (.01 degrees C) in the earth's month earth temperature, given that co2 
...
#H0: there is no greenhouse gas effect => beta1= 0 and beta2= 0

#Ttests aren't good at this. Let's use ANOVA
#ANOVA compares the two models 1. Assuming H0 is true and 2. Assume Ha is true (climate.out). 
#If there is a difference between the two models, we'll reject H0. 

#Test H0: Neither greenhouse gas has an effect
nogreenhousegas <- lm(temp ~ +1, data=climate) #+1 puts y intercept in model. 
                                                #It is in normal lm() by default
anova(nogreenhousegas, climate.out)

#There is a statistically significant greenhouse gas effect (p is < 2.2e-16)
...
#can get pval out of 
summary (climate.out)
#           test statistic  Pvalues
#co2           7.478        4.79e-13 ***
#methane       2.880        0.00419 ** 
  
# 95% Confindence Interval
confint(climate.out)

#When we compared the two models with the ANOVA test, the pvalue was less than .0001, and so
#we reject the null that methane and co2 do not have a greenhouse gas effect. In our further 
#analysis, we tested co2 and methane individually to see if each gas individually had a 
#greenhouse gas effect. In our ttest Beta1 had a t statistic of 7.478, with a pvalue 4.79e-13,
#and therefore CO2 has a significant effect. Beta2 had a statistic of 2.880, with a pvalue of 
#0.00419, and so methane also has a statistically signficant #greenhouse gas effect. Methane's 
#effect was not as significant as the effect of CO2, because Methane's statistic was smaller.
#These findings were reinforced by the confidence interval that we calculated. Neither co2 or
#methane contained 0 in their respective 95% confidence intervals, and therefore both are 
#statistically significant in that regard. Methane had a much smaller confidence interval than
#co2, which shows again that it is less significant than co2.

#Evaluate prediction performance one year into the future
#Prediction: How good? Model ok?
...
#Predict temp on test dataset
#evaluate prediciton performance
predict(out.train.climate, newdata=climate.test)

cbind(climate.train$temp, predict(out.train.climate, newdata = climate.test))
